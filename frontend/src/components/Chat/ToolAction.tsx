...
export default function ToolAction({ toolCall, pending }: Props) {
  ...
  return (
    <div
      className="animate-slide-up flex items-center gap-3 rounded-xl px-4 py-2.5"
    ...
    >
      ...
      {toolCall.tool_input && Object.keys(toolCall.tool_input).length > 0 && (
        <span className="text-[12px] truncate ml-auto" style={{ color: 'var(--text-muted)', maxWidth: 240 }}>
          {Object.entries(toolCall.tool_input).slice(0, 2).map(([k, v]) =>
            `${k}: ${typeof v === 'string' ? v.slice(0, 24) : JSON.stringify(v)}`
          ).join(', ')}
        </span>
      )}
    </div>
